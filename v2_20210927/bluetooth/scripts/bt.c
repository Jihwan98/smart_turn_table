...
void sigint_handler(int signo){
    int shutdown(int sock, int SHUT_RDWR);
...
void *ThreadMain(void *argument)
{
    ...
    while(1)
    {
        ...
        if (index == 0){
            strcpy(email_id, recv_message);}
        else if (index == 1){
            strcpy(wifi_id, recv_message);}
        else{
            strcpy(wifi_pw, recv_message);}
        index++;
    }
    printf("email id : %s\n", email_id);
    printf("wifi id : %s\n", wifi_id);
    ...
    FILE * file;
    file = fopen("user_email.txt", "w+");
    if(file == NULL){
        printf("email_id file open orror\n");}
    else{
        printf("user_email.txt created\n");
        fprintf(file, "email_id=%s", email_id);
