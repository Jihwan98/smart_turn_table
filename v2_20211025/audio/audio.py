...
class VADAudio(Audio):
    ...
    def vad_collector(self, padding_ms=300, ratio=0.75, frames=None):
        ...
        for idx, f32, f32_1c, i16, i16_1c in frames:
            ...
            yield ssl_frame, tc_frame
    def change_aggressiveness(self, aggressiveness):
        self.vad = webrtcvad.Vad(aggressiveness)

def audio_thread(audio2SSL_audio, audio2TC_audio, loading, rainbow, device_mode):
    ...
    for ssl, tc in vad.vad_collector():
        if device_mode.queue[-1] != pre_mode:
            if device_mode.queue[-1] == 'security':
                vad.change_aggressiveness(0)
                print("change aggressiveness to 0")
                pre_mode = 'security'
            else:
                vad.change_aggressiveness(2)
                print("change aggressiveness to 2")
                pre_mode = 'normal'
