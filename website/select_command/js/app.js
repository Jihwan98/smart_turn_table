    ...
    if (user) {
        ...
        document.getElementById("select").onclick = function(e){
            ...
            input.onchange = e => {
                ...
                document.getElementById("file_num").innerHTML = "선택한 파일 개수 :" + files.length
            }
            input.click();
        }
        ...
        document.getElementById('upload').onclick = function(){
            if (files[0]){
                for (i = 0; i < files.length; i++){
                    ...
                    var uploadTask = firebase.storage().ref('Voices/'+email+'/'+cmd_text + "/" + files[i].name).put(files[i], metadata); // firestore에서 다운로드가 되고
                    ...
                    //         Command_Voice_Name : cmd_text + "_" + files[i].name,
                    //         Update_Time : Date()
                    //     }, {merge : true});
                    //     });
                    // }
        ...
        document.getElementById('cmd_yes').onclick = function(){
            ...
            if(Command_text_btn == "시리야"){
                Command_text_btn_2 = 'siriya';
            }
            else if(Command_text_btn == "호반아"){
                Command_text_btn_2 = 'hobanah';
            }
            else{
                Command_text_btn_2 = Command_text_btn
            }
            console.log("Your Command : " + Command_text_btn_2);
            
            docRef.set({
                Command_text : Command_text_btn_2,
                Update_Time : Date()
            }, {merge : true});
