...
def get_at():
    ...
    at_a = angle_table_base
    at_b = (angle_table_base + 25.71) % 360
...
class SpecialBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, f_size):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(in_channels, out_channels*2, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, x):
        ...
        out =  torch.cat((out, out1, out2, out3), 1)
        return out
...
class SounDNet(nn.Module):
    def __init__(self, frame_len=2560):
        ...
        self.fc_layer = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(128, self.num_classes*2),
        )
...
#print(torch.max(F.softmax(pred_label)))
    pa = pred_angle[0][p_idx] * 25.71
    if pa > 25.71:
        pa = 25.71
    elif pa < 0:
        pa = 0
    p_angle = dict_label[p_idx] + pa
...
def ssl_thread(ssl_queue, angle_list, trig_ent, new_ent):
    ...
    net = SounDNet()
    net.load_state_dict(torch.load('sLocalization/weight/138.pth', map_location=torch.device('cpu')))
    net.eval()
    ...
    while True:
        ...
        #print(torch.mean(torch.abs(sample)))
        if torch.mean(torch.abs(sample)) < 0.003:
            continue
...
if __name__ == '__main__' :
    ...
    net = SounDNet()
    net.load_state_dict(torch.load('weight/476.pth', map_location=torch.device('cpu')))
    net.eval()        
    go = 0
    for i, (frame,_) in enumerate(ssl_frames):
        #print(len(ssl_frames))
        if i % 2 == 0:
            pre = frame[1]
        else:
            data = np.concatenate((pre, frame[1]), 0)
            t_data = torch.tensor(data).view(-1,640,8).permute(0, 2, 1).contiguous().float()
