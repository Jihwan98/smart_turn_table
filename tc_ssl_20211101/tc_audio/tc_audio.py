...
def tc_audio_thread(tc_frame_que, tc_audio_que, device_mode):
    ...
    while True:
        ...
        ring_buffer.append((f32_1c, f32, is_speech))
        rb_len += 1
        ...
        if dev_mode == "security" :
            ...
            if rb_len == maxlen :
                ...
                for i, (f, f32, s) in enumerate(ring_buffer):
                    if i == 0:
                        sample = f
                        sample2 = f32
                    else :
                        sample = np.concatenate((sample, f), axis = 0)
                        sample2 = np.concatenate((sample2, f32), axis=0)
            ...
            maxlen = 50
            if rb_len == maxlen:
                num_voiced = len([f for f, f32, speech in ring_buffer if speech])
