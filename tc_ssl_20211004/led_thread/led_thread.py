...
def rainbow_led(rainbow, device_mode, loading):
    pre_mode = device_mode.queue[-1]
    everloop = ['black'] * led.length
...
def led_thread(internet_ent, internet, loading, device_mode, rainbow):
    ...
    while True:
        ...
        if now_mode != past_mode:
            if now_mode != 'training' and past_mode != 'training':
                twinkle()
            past_mode = now_mode
        ...
        if rainbow.queue[-1] == 1:
            rainbow_led(rainbow, device_mode, loading)
